...
console.log(JSON.parse(sessionStorage.getItem('offerParaJson')));
		$(document).ready(function(){
			var huanchenhtml = '<div class="requirement requirement3 huanchenpage" type="3">'+
					...
					'</div>'+
					'<div class="papercol special-tech add-special-div" style="cursor: pointer;">'+
						'<font>特殊工艺</font>'+
...
//					$('.huanchenpage').remove()
			...
			if(offerParaJson.binging==undefined || offerParaJson.binging.bindStyle==undefined){
					console.log(2222)
...
//					$('.size-paper option').eq(i).attr('selected','selected');
//				}else{
//					$('.size-paper option').eq(i).removeAttr('selected');
//				}
//			}
			if(offerParaJson.sizestr==undefined){
				$('.requirement3 .pagesizenum').html();
			$('.requirement3 .pagesizenum').attr('pagewidth','');
			$('.requirement3 .pagesizenum').attr('pageheight','');
			}else{
				$('.requirement3 .pagesizenum').html('尺寸:'+(offerParaJson.sizestr.pageWidth==undefined?'':offerParaJson.sizestr.pageWidth)+'X'+(offerParaJson.sizestr.pageHeight==undefined?'':offerParaJson.sizestr.pageHeight));
			$('.requirement3 .pagesizenum').attr('pagewidth',(offerParaJson.sizestr.pageWidth==undefined?'':offerParaJson.sizestr.pageWidth));
			$('.requirement3 .pagesizenum').attr('pageheight',(offerParaJson.sizestr.pageHeight==undefined?'':offerParaJson.sizestr.pageHeight));
			}
...
//			alert(offerParaJson.linling.length+'测试长度');
			...
			if(offerParaJson.inside==undefined){
			...
			for (var i = 0; i < offerParaJson.linling.length; i++) {
				console.log(offerParaJson.linling[i].selfPaperPrice);
				var coverhtml = '<div class="requirement requirement3 coverpage coverpages" type="1">';
				console.log(i>0);
				
				
					
				if(offerParaJson.linling[i].selfPaperPrice=='0'||offerParaJson.linling[i].selfPaperPrice=='undefined' || offerParaJson.linling[i].selfPaperPrice==null || offerParaJson.linling[i].selfPaperPrice==undefined){
					if(i==0){
					...
					coverhtml = coverhtml+'<select class="paper-keZhong">'+
							'<option value="" selected="selected" selfPaperPrice="'+offerParaJson.linling[i].selfPaperPrice+'">'+offerParaJson.linling[i].selfPaperWeight+'</option>'+
						'</select>'+
						'<span>克</span>'+
						'<input type="text" value="'+offerParaJson.linling[i].selfPaperName+'" class="paper-type" readonly="readonly" status="cover"/>'+
					'</div>';
				}
		...
		layer.open({
			...
			console.log($('.coverpages:last-child')+'测试')
				...
				if(offerParaJson.huanchen!=undefined){
					...
					if(offerParaJson.huanchen.length==0){
						...
						if(offerParaJson.binging.bindStyle=='硬面精装' || offerParaJson.binging.bindStyle=='软面精装'){
							...
							'<p style="padding: 0px 15px;color: #8e8e8e;" class="del">环衬</p>'+
							'<div class="papercol" id="lining" style="border: 0px;">无环衬(自衬)'+
								'<input type="checkbox" name="lining" class="input_check" id="choose" checked="checked"/>'+
						        '<label for="choose"></label>'+
							'</div>'+
							'<div class="papercol paper papers-type" status="lining">'+
								'<font>环衬纸张</font><select class="paper-keZhong"><option value=""  selected="selected"></option>'+
					...
					'<div class="papercol paper papers-type" status="lining">'+
						...
						if(offerParaJson.huanchen[i].isSelfPaper==0 || offerParaJson.huanchen[i].isSelfPaper==null){
							html = html+'<select class="paper-keZhong"><option value="" price="'+(offerParaJson.huanchen[i].dunWeiJia==undefined?'':offerParaJson.huanchen[i].dunWeiJia)+'" selected="selected">'+(offerParaJson.huanchen[i].keZhong==undefined?'':offerParaJson.huanchen[i].keZhong)+'</option>'+
							'</select><span>克</span>'+
						...
						}else if(offerParaJson.huanchen[i].isSelfPaper = 1){
							html = html+'<select class="paper-keZhong"><option value="" selfPaperPrice="'+offerParaJson.huanchen[i].selfPaperPrice+'" selected="selected">'+offerParaJson.huanchen[i].selfPaperWeight+'</option>'+
							'</select><span>克</span>'+
							'<input type="text" value="'+offerParaJson.huanchen[i].selfPaperName+'" class="paper-type" readonly="readonly"  status="lining" />';
						}
				...
				html = html+'</div>'+
					...
					'<div class="papercol pages">'+
						'<font>环衬页数</font>'+
						'<input type="number" class="pages-num" value="'+offerParaJson.huanchen[i].pagesNum+'" />P'+
					'</div>'+
			...
			for (var i = 0; i < offerParaJson.inside.length; i++) {
				...
				if(i==0){
					...
					'<div class="papercol paper papers-type" status="lining">'+
						...
						if(offerParaJson.inside[i].isSelfPaper==0 || offerParaJson.inside[i].isSelfPaper==null){
						...
						}else if(offerParaJson.inside[i].isSelfPaper = 1){
							html = html+'<select class="paper-keZhong"><option value="" selfPaperPrice="'+offerParaJson.inside[i].selfPaperPrice+'" selected="selected">'+offerParaJson.inside[i].selfPaperWeight+'</option>'+
							'</select><span>克</span>'+
							'<input type="text" value="'+offerParaJson.inside[i].selfPaperName+'" class="paper-type" readonly="readonly"  status="lining" />';
						}
...
//				$('.container').find('.requirement').eq(index).css('display','block');
				...
				'<div class="papercol surface-handle handle" index="'+(Number(index)+1)+'">'+
					...
					'<input type="text" class="val" value="" style="width: 35%;"/>'+
				'</div>'+
				'<div class="papercol special-tech add-special-div">'+
					'<font>特殊工艺</font>'+
					'<img src="img/add.png" class="add-icon" />'+
...
//							console.log($('this').parent().siblings().find('.add-papers-icon').attr('index'));
				...
				$('.requirement3').off().on('tap','.papers-type',function(e){
					e.preventDefault();
					e.stopPropagation()
