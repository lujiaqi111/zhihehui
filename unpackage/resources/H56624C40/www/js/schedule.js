				...
				el.addEventListener('click', function(e) {
					...
					if(e.target.className.indexOf('currentDate') > -1) {
						...
						$('.schedule-bd li').on('tap',function(){
							...
							$(this).find('span').addClass('zc_day_active');
							$(this).find('span').removeClass('zc_day');
							$.ajax({
											type:"post",
...
//			}
		...
		var init = function() {
			var scheduleHd = '<div class="schedule-hd">' +
				'<div style="width: 50px; text-align: right;" id="prevMonth">' +
				'<span class="arrow icon iconfont icon-112leftarrowhead" style="-webkit-tap-highlight-color:rgba(0,0,0,0);"></span>' +
				'</div>' +
				'<div class="today">' + year + "年" + (month + 1) + "月" + '</div>' +
				'<div style="width: 50px; text-align: left;" id="nextMonth">' +
				'<span class="arrow icon iconfont icon-111arrowheadright" style="-webkit-tap-highlight-color:rgba(0,0,0,0);" ></span>' +
				'</div>' +
				'</div>'
